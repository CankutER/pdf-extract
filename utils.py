...
def is_capitalized_or_uppercase(text):
    ...
    result = all(w[0].isupper() or w.isupper()
                 for w in filtered_words) and len(filtered_words) <= 10
    return result


def safe_filename(filename: str) -> str:
    # Remove path components (prevents ../../)
    filename = os.path.basename(filename)

    # Normalize unicode (important for Turkish chars consistency)
    filename = unicodedata.normalize("NFKC", filename)

    # Remove control characters
    filename = re.sub(r'[\x00-\x1f\x7f]', '', filename)

    # Replace forbidden filesystem chars with _
